...
def _rank(results, season, episode, mode="video", requested=None, **kw):
    ranked, trace = selector_tv.rank_for_episode(
        results, mode, "Serie Exemplo", "2008", season, episode,
...
def test_cobre_o_episodio_ou_rejeita():
    results = [
        cand("Serie Exemplo S01E02 1080p WEB-DL"),
        cand("Serie Exemplo S01E03 1080p WEB-DL"),
        cand("Serie Exemplo S02 1080p WEB-DL"),
    ]
    ranked, trace = _rank(results, 1, 2)
    assert [r["title"] for r in ranked] == ["Serie Exemplo S01E02 1080p WEB-DL"]
    rejected = {c["title"]: c["rejected"] for c in trace}
    assert "não cobre S01E02" in rejected["Serie Exemplo S01E03 1080p WEB-DL"]
    assert "não cobre S01E02" in rejected["Serie Exemplo S02 1080p WEB-DL"]


def test_pack_cobre_episodio():
    results = [cand("Serie Exemplo S01 1080p BluRay")]
...
def test_serie_completa_cobre_com_known_seasons():
    results = [cand("Serie Exemplo Complete Series 1080p")]
...
def test_titulo_de_outra_serie_rejeita():
    results = [cand("Outra Serie S01E02 1080p WEB-DL")]
...
def test_qualidade_primeiro():
    ...
    results = [
        cand("Serie Exemplo S01 720p WEB-DL"),
        cand("Serie Exemplo S01E02 1080p BluRay"),
    ]
    ranked, _ = _rank(results, 1, 2, requested=[(1, 1), (1, 2), (1, 3)])
    assert ranked[0]["title"] == "Serie Exemplo S01E02 1080p BluRay"
...
def test_pack_desempata_no_mesmo_tier():
    ...
    results = [
        cand("Serie Exemplo S01E02 1080p WEB-DL"),
        cand("Serie Exemplo S01 1080p WEB-DL"),
    ]
    ranked, _ = _rank(results, 1, 2, requested=[(1, 1), (1, 2), (1, 3)])
    assert ranked[0]["title"] == "Serie Exemplo S01 1080p WEB-DL"


def test_audio_exige_marcador_e_rejeita_legendado():
    results = [
        cand("Serie Exemplo S01E02 1080p Dublado PT-BR"),
        cand("Serie Exemplo S01E02 1080p Legendado"),
        cand("Serie Exemplo S01E02 1080p"),
    ]
    ranked, trace = _rank(results, 1, 2, mode="audio", language="pt",
                          require_language=True)
    assert [r["title"] for r in ranked] == ["Serie Exemplo S01E02 1080p Dublado PT-BR"]
    rejected = {c["title"]: c["rejected"] for c in trace}
    assert rejected["Serie Exemplo S01E02 1080p"] is not None


def test_marcador_forte_vence_qualidade_no_audio():
    # dublagem confirmada (forte) vence 4K com marcador fraco — como em filmes
    results = [
        cand("Serie Exemplo S01E02 2160p WEB-DL dual"),
        cand("Serie Exemplo S01E02 720p WEB-DL Dublado"),
    ]
    ranked, _ = _rank(results, 1, 2, mode="audio", language="pt")
    assert ranked[0]["title"] == "Serie Exemplo S01E02 720p WEB-DL Dublado"


def test_trace_tem_coverage_e_shape_de_filme():
    results = [cand("Serie Exemplo S01 1080p WEB-DL")]
    _, trace = _rank(results, 1, 2)
    c = trace[0]
...
def _ranked_map(requested, results, **kw):
    ...
    for s, e in requested:
        ranked, _ = selector_tv.rank_for_episode(
            results, "video", "Serie Exemplo", "2008", s, e,
            requested=requested, **kw)
        out[(s, e)] = ranked
    return out
...
def test_plan_reusa_pack():
    ...
    results = [
        cand("Serie Exemplo S01 1080p WEB-DL", size=9_000_000_000),
        cand("Serie Exemplo S01E02 1080p WEB-DL"),
    ]
...
def test_plan_avulso_melhor_nao_e_sacrificado():
    ...
    assert plan["assignments"][(1, 2)]["title"] == "Serie Exemplo S01E02 1080p BluRay"
    assert plan["assignments"][(1, 1)]["title"] == "Serie Exemplo S01 1080p WEB-DL"


def test_plan_marca_lacunas():
    requested = [(1, 1), (1, 99)]
    results = [cand("Serie Exemplo S01E01 1080p WEB-DL")]
    plan = selector_tv.build_plan(requested, _ranked_map(requested, results))
