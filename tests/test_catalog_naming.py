...
def item(temp_db, tmp_path):
    """Um destino com a pasta 'Filme Exemplo (2014)'. Devolve (dest_id, root)."""
    root = tmp_path / "colecao"
    root.mkdir()
    dest = temp_db.add_destination("T", str(root), True)
    (root / "Filme Exemplo (2014)").mkdir()
    return dest["id"], root


def test_folder_name():
    assert catalog.folder_name("Filme Exemplo", "2014", 1111) == "Filme Exemplo (2014) [tmdbid-1111]"
    assert catalog.folder_name("Filme Exemplo", "2014") == "Filme Exemplo (2014)"  # sem id: como sempre foi
    assert catalog.folder_name("Sem Ano", None, 5) == "Sem Ano [tmdbid-5]"
    # caracteres proibidos em nome de pasta somem (o id fica intacto)
    assert catalog.folder_name('Filme: O 8º Passageiro', "1979", 2222) \
        == "Filme O 8º Passageiro (1979) [tmdbid-2222]"


def test_tmdb_id_in():
    assert catalog.tmdb_id_in("Filme Exemplo (2014) [tmdbid-1111]") == 1111
    assert catalog.tmdb_id_in("Filme Exemplo (2014)") is None
...
def test_title_and_year_ignores_tmdbid():
    ...
    assert catalog._title_and_year("Filme Exemplo (2014) [tmdbid-1111]") == ("Filme Exemplo", "2014")
...
def test_library_key_matches_with_tmdbid(item, temp_db):
    ...
    _did, root = item
    (root / "ROBO-X (2008) [tmdbid-3333]").mkdir()
    catalog.invalidate_library()
    keys = catalog.library_keys()
    assert ("robox", "2008") in keys
    assert catalog.in_library({"original_title": "ROBO·X", "title": "ROBO·X", "year": "2008"}, keys)


def test_tag_folder_with_tmdb(item):
    did, root = item
    novo = catalog.tag_folder_with_tmdb(did, "Filme Exemplo (2014)", 1111)
    assert novo == "Filme Exemplo (2014) [tmdbid-1111]"
    assert (root / novo).is_dir()
    assert not (root / "Filme Exemplo (2014)").exists()
...
def test_tag_folder_invalidates_library(item):
    ...
    catalog.tag_folder_with_tmdb(did, "Filme Exemplo (2014)", 1111)
    assert catalog._library_cache["at"] is None  # o nome da pasta mudou


def test_tag_folder_collision(item):
    did, root = item
    (root / "Filme Exemplo (2014) [tmdbid-1111]").mkdir()
...
def test_rename_folder_rejects_escape(item):
    ...
    with pytest.raises(catalog.CatalogError):
        catalog.rename_folder(did, "Filme Exemplo (2014)", "../fora")
    assert (root / "Filme Exemplo (2014)").is_dir()  # nada se moveu


def test_media_path(item, tmp_path):
    did, root = item
    folder = "Filme Exemplo (2014)"
