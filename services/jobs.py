...
async def _run_manual(job: dict, video_file: Path, audio_file: Path):
    ...
    try:
        ...
        await _pause_for_drift(job, {"video": video_file, "audio": audio_file}, e)
...
async def _run_from_download(job: dict):
    try:
        ...
        await _pause_for_drift(job, files, e)  # já fora do lock: a fila fica livre
    except Exception as e:  # noqa: BLE001
        _fail(job, f"{type(e).__name__}: {e}")
...
# -------------------- alinhamento avançado (EDL) em filmes --------------------
